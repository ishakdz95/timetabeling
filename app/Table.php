...
{
    public function make_random_table(){
        ...
        foreach ($professors as $professor) {
            ...
            while ($x < 6 && $bool == false) {
                ...
                if ($available == true) {
                    ...
                    $timetabling1->day_id=$timeslot->day->id;
                    $timetabling1->day_name=$timeslot->day->name;
                    ...
                    $timetabling1->group_id = $seances[$random]->group_id;
                    $timetabling1->group_name = $seances[$random]->group_name;
    ...
    public function fitness_function(array $tables){
        $groups=Group::all();
        foreach ($tables as $table){
            foreach ($table as $value){
                foreach ($table as $value2){
                    if ($value->group_id==$value2->group_id || $value->section_id==$groups->find($value2->group_id)->section_id ){
